...
Route::prefix('performance')->group(function () {
    ...
    Route::get('/slow-queries', [PerformanceController::class, 'slowQueries'])
        ->name('api.performance.slow-queries');
    Route::post('/clear-metrics', [PerformanceController::class, 'clearMetrics'])
        ->name('api.performance.clear-metrics');
});
...
Route::prefix('messages')->group(function () {
    Route::post('/send', [Api\MessageController::class, 'sendMessage'])
        ->middleware('throttle:20,1'); // 20 messages per minute to prevent spam
...
Route::prefix('messages')->name('api.messages.')->group(function () {
    // Send message (buyer to vendor or vendor to buyer)
    ...
    // Get messages for a specific quote
